...
@isg.lazy_config
def build_platform_dir(build_dir, host_platform):
...
@isg.lazy_config
def build_pythonscript_dir(build_platform_dir):
...
def compile_pythonscript_c(
    ...
    cmd = f"{cc} {' '.join(cflags)} {' '.join(extra_cflags)} -o {output} -c {input}"
    print(cmd)
